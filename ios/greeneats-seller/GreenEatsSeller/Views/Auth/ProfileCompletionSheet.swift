...
/// Captures name/email/phone after Apple sign-in when Apple gave us an empty
/// `fullName` or a @privaterelay.appleid.com forwarder. Presented modally over
/// the seller root view and disabled from interactive dismiss — sellers need a
/// real name to appear on orders, so completing this step is required.
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text("We need your name and a reachable email to associate with your restaurant.")
                            .font(.body)
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        labeledField("Email") {
                            TextField("you@example.com", text: $email)
                                ...
                                .keTextFieldStyle()
                            if isRelayEmail {
                                Text("Apple gave us a forwarding address. Enter the email you'd like order and payout notifications sent to.")
                                    .font(.caption)
                                    .foregroundColor(.keTextSecondary)
                            }
                        }
                    ...
                    // TEMPORARY — App Review escape hatch. Sellers can't
                    // actually transact without a name/email on file, so this
                    // should be removed once the app is live and reviewers
                    // aren't driving the Apple sign-in flow blind.
