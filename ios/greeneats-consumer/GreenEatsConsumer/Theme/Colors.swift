...
extension Color {
    // MARK: - Brand (green + baby blue, GreenEats palette)
    // kePrimary = leaf green (#22C55E base, deepened for punch)
    // kePrimaryLight = baby blue accent that pairs with the green
    // Kept as `kePrimary` etc. so all downstream views referencing the
    // ke* tokens flip palette automatically — name kept for parity.
    static let kePrimary = Color(hex: "16A34A")        // green-600
    static let kePrimaryLight = Color(hex: "A5D8FF")   // baby blue
    static let kePrimaryDark = Color(hex: "15803D")    // green-700
    ...
    // MARK: - Diet badges (GreenEats — vegan/vegetarian/raw)
    // Tokens keep their KE-prefixed names so consumer-app code that
    // references them by name still compiles; the values are recoloured
    // to fit the new palette (green + baby blue with a warning amber).
    static let keMeat = Color(hex: "F59E0B")   // amber — "vegetarian" (allows dairy/egg)
    static let keDairy = Color(hex: "3B82F6")  // blue — repurposed if dairy substitute UI keeps the token
    static let kePareve = Color(hex: "22C55E") // green — "100% vegan"
