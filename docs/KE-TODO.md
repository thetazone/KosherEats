...
## ✅ Two deferred high-sev backend bugs — FIXED & DEPLOYED (2026-06-25, commit `0418991b`)
Both deferred items closed, with an adversarial-verification pass that caught 4 follow-on gaps (all fixed before deploy). Migration 051 verified live on prod; backfill left 0 historical orders unsettled; `/health` 200. 2 new regression tests.
- [x] **Refund-before-commit atomicity** (CancelOrder, RejectOrder, tryStaleReject): refund was issued before the status commit → a commit failure left a refunded-but-fulfillable order (free food). Now flip + COMMIT first, then refund; a post-commit refund failure is recovered by a new reconcile reaper (`sweepPendingRefunds`). Added `orders.refunded_at` + `refund_attempts` (migration 051, backfilled). `RefundPaymentIntent` is now idempotent (swallows `charge_already_refunded` + stable per-PI idempotency key → no concurrent double-refund); 3-min reaper grace; 10-attempt poison-pill cap with a manual-reconciliation alert.
- [x] **Password-reset cross-account**: not a code-theft takeover (the code is always emailed to the row's own address), but the legacy email-only fallback could resolve to an unverified phone/OAuth squatter row, and the role-scoped path was non-deterministic under a case-collision. Both lookups (legacy + role-scoped) now filter to real password accounts (`auth_provider` NULL/'email') + order deterministically. Email is lowercased on insert in `createPhoneUser`, `CourierRegister`, `AdminCreateSeller` (Register already did) to stop case-variant duplicate/squatter rows.
- **Future hardening noted:** make `idx_users_email_role_vertical` case-insensitive (`lower(email)`) as the root-cause fix for email-case duplicates — deferred (needs a prod case-dupe check first, like the courier index).
...
- [x] **#2 Payout idempotency (Temporal cutover)** — **already aligned; nothing to do at cutover** (re-verified 2026-07-29). Both paths key the Stripe idempotency key on the `courier_payout_queue` row id: legacy passes `p.id`, Temporal resolves `SELECT id::text FROM courier_payout_queue WHERE order_id = $1`. The old instruction here (and the matching TODO at the legacy payout site) said to align both on `payout-`+orderID — that was based on a false reading of the Temporal path, and doing it would have made the two disagree and **created** the double-pay. Both have been corrected. ⚠️ Separate, still-open payout risk: the retry backoff's final attempt lands past t+31h while Stripe drops idempotency keys at 24h, so a succeeded-but-errored transfer can still be replayed. See the comment at `tryPayout`.
