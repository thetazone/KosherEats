...
- **Deferred (1, needs a coordinated change):** public restaurant endpoints expose `owner_id` to anonymous callers — the `json:"-"` fix would break a shipped iOS app that does a required decode, so it needs a split public/seller DTO + an app update (make the field optional). Tracked, not auto-fixed.
- **Client builds NOT shipped to TestFlight/Play** — that remains Salto's release step.
