...
- ⏳ **Client highs remaining:** iOS — self-delivery seller can't pickup/deliver its own orders (missing endpoints; needs `delivery_mode` on the order payload → small backend change), in-flight PaymentIntent can permanently lock checkout (needs timestamp/expiry), default-modifier preselect ignores single-select/availability. Android — courier card shows payout without tip, default %-tip dropped to $0 on process-death recovery, menu sheet doesn't preselect default modifiers. Then: **consumer Uber-delivery tracking** (phase 2 feature) → **flagged backend** (below).
