...
private fun darkScheme(c: GreenEatsColors): ColorScheme = darkColorScheme(
    primary = Orange,
...
private fun lightScheme(c: GreenEatsColors): ColorScheme = lightColorScheme(
    primary = Orange,
    onPrimary = Color.White,
...
fun GreenEatsTheme(
    ...
    val greeneatsColors = if (darkTheme) DarkGreenEatsColors else LightGreenEatsColors
    val colorScheme = if (darkTheme) darkScheme(greeneatsColors) else lightScheme(greeneatsColors)

    val view = LocalView.current
    if (!view.isInEditMode) {
        val barColor = greeneatsColors.backgroundBlack.toArgb()
    ...
    CompositionLocalProvider(LocalGreenEatsColors provides greeneatsColors) {
        MaterialTheme(
            colorScheme = colorScheme,
            typography = GreenEatsTypography,
            content = content,
        )
    }
}
