...
// Where to land after a successful sign-in. ?next= carries the page that sent
// the user here — a restaurant page mid-order, or /cart from checkout's
// pending-order recovery, which must resume there after a forced re-login —
...
function AuthFlow() {
  ...
  // Where to land after sign-in/sign-up.
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      storeSession(data);
      // Full navigation (not router.replace) so the Header re-reads the
      // stored session; honors ?next= to resume an interrupted flow (e.g.
      // adding an item on a restaurant page).
    ...
    <div className="w-full max-w-md">
      ...
      {info && (
        <div
          role="status"
          aria-live="polite"
          className="bg-dark-800 border border-dark-700 text-dark-300 rounded-xl px-4 py-3 mb-6 text-sm"
        >
          {info}
        </div>
      )}
      ...
      {step === "password" && (
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label htmlFor="signin-email" className="block text-sm text-dark-300 mb-1.5">
              Email
            </label>
            <input
              id="signin-email"
