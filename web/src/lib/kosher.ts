...
// --- Preview-listing helpers (carried over from main's preview-listing work) -
//
// These narrow isPlaceholderCertification into the three cases the browse and
// detail surfaces actually distinguish:
//   * a real, displayable certification  -> certLabel returns the string
//   * a placeholder on a restaurant that HAS a cert field filled in (the
//     importer's "TBD - to be provided by merchant") -> "pending" treatment
//   * an EMPTY cert (preview listings seeded before the owner onboards, with
//     no hashgacha on file at all) -> render no certification chip/row at all
// The empty case matters: a preview listing showing "Certification pending"
// implies an application in flight that may not exist.

/** The certification to display, or null when there is nothing real to show. */
export function certLabel(cert: string | null | undefined): string | null {
  const trimmed = (cert ?? "").trim();
  if (!trimmed || isPlaceholderCertification(trimmed)) return null;
  return trimmed;
}

/** True for "onboarding in flight" — a non-empty cert with no real label. */
export function certIsPending(cert: string | null | undefined): boolean {
  const trimmed = (cert ?? "").trim();
  return trimmed !== "" && certLabel(trimmed) === null;
}

/** True when there is no certification field on file at all (preview seeds). */
export function certIsAbsent(cert: string | null | undefined): boolean {
  return (cert ?? "").trim() === "";
}
